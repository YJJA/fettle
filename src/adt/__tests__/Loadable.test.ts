...
test("Error Loadable", async () => {
  const loadable = Loadable.withError(ERROR);
...
test("Pending Value Loadable", async () => {
  ...
  const loadable = Loadable.withPromise(promise);
...
describe("Loadable mapping", () => {
  test("Loadable mapping value", () => {
    const loadable = Loadable.withValue("VALUE").map((x) => "MAPPED " + x);
    expect(loadable.state).toBe("hasValue");
    expect(loadable.contents).toBe("MAPPED VALUE");
  });

  test("Loadable mapping value to error", () => {
    const loadable = Loadable.withValue("VALUE").map(() => {
      throw ERROR;
    });
    expect(loadable.state).toBe("hasError");
    expect(loadable.contents).toBe(ERROR);
  ...
  test("Loadable mapping value to Promise", async () => {
    const loadable = Loadable.withValue("VALUE").map((value) =>
  ...
  test("Loadable mapping error", () => {
    const loadable = Loadable.withError(ERROR).map(() => "NOT_USED");
    expect(loadable.state).toBe("hasError");
    expect(loadable.contents).toBe(ERROR);
  });

  test("Loadable mapping promise value", async () => {
    const loadable = Loadable.withPromise(Promise.resolve("VALUE")).map(
      (x) => "MAPPED " + x
    );
    expect(loadable.state).toBe("loading");
    await expect(loadable.toPromise()).resolves.toBe("MAPPED VALUE");
  ...
  test("Loadable mapping promise error", async () => {
    const loadable = Loadable.withPromise(Promise.reject(ERROR)).map(
      () => "NOT_USED"
    );
    expect(loadable.state).toBe("loading");
    await expect(loadable.toPromise()).rejects.toBe(ERROR);
  ...
  test("Loadable mapping promise to loadable value", async () => {
    const loadable = Loadable.withPromise(Promise.resolve("VALUE")).map(
      (value) => Loadable.withValue("MAPPED " + value)
    );
    expect(loadable.state).toBe("loading");
    await expect(loadable.toPromise()).resolves.toBe("MAPPED VALUE");
  });
  ...
  test("Loadable mapping promise to loadable promise", async () => {
    const loadable = Loadable.withPromise(Promise.resolve("VALUE")).map(
      (value) => Loadable.withPromise(Promise.resolve("MAPPED " + value))
    );
    expect(loadable.state).toBe("loading");
    await expect(loadable.toPromise()).resolves.toBe("MAPPED VALUE");
  });
...
describe("Loadable All", () => {
  test("Array", async () => {
    ...
    expect(
      Loadable.all([
        Loadable.of("x"),
        Loadable.all([Loadable.of(1), Loadable.of(2)]),
      ]).contents
    ).toEqual(["x", [1, 2]]);
  });

  test("mixed values", async () => {
    expect(Loadable.all([Loadable.of("A"), "B"]).contents).toEqual(["A", "B"]);

    await expect(
      Loadable.all([Loadable.of("A"), Promise.resolve("B")]).contents
    ).resolves.toEqual(["A", "B"]);

    await expect(
      Loadable.all([Loadable.of("A"), Promise.reject("B")]).contents
    ).rejects.toEqual("B");
  });
});

describe("Loadable AllSettled", () => {
  test("Array", async () => {
    expect(
      Loadable.allSettled([Loadable.of("x"), Loadable.of(123)]).contents
    ).toEqual([
    ...
    await expect(
      Loadable.allSettled([Loadable.of(Promise.resolve("x")), Loadable.of(123)])
        .contents
    ).resolves.toEqual([
      { status: "fulfilled", value: "x" },
      { status: "fulfilled", value: 123 },
  ...
  test("mixed values", async () => {
    expect(Loadable.allSettled([Loadable.of("A"), "B"]).contents).toEqual([
      { status: "fulfilled", value: "A" },
      { status: "fulfilled", value: "B" },
    ]);

    await expect(
      Loadable.allSettled([Loadable.of("A"), Promise.resolve("B")]).contents
    ).resolves.toEqual([
      { status: "fulfilled", value: "A" },
      { status: "fulfilled", value: "B" },
    ]);
    ...
    await expect(
      Loadable.allSettled([Loadable.of("A"), Promise.reject("B")]).contents
    ).resolves.toEqual([
      { status: "fulfilled", value: "A" },
